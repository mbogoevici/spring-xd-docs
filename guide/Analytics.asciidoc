...
An in memory implementation and a Redis implementation are provided in *Spring XD 1.0.0.M2*.  Other metrics that will be provided in a future release are Rate Counters and Histograms.
...
Next, create a tap on the _springtweets_ stream that sets a message counter named _tweetcount_
   ...
   xd:> stream create --name tweettap --definition "tap:springtweets > counter --name=tweetcount"
...
Now create a tap for a field value counter:
   ...
   xd:> stream create --name tweettap --definition "tap:springtweets > field-value-counter --fieldName=fromUser"
...
you'd simply create a tap which pipes the input to `aggregatecounter`:
   ...
   xd:> stream create --name tweettap --definition "tap:springtweets > aggregatecounter --name=tweetcount"
...
Next create the tap:
    ...
    xd:> stream create --name simplegauge --definition "tap:test > gauge"
...
Next create the tap:
      ...
      xd:> stream create --name testgauge --definition "tap:test > richgauge"
...
Next create the tap, using the json-field-extractor to extract the stock price from the payload: 
     ...
     xd:> stream create --name stockprice --definition "tap:stocks > json-field-extractor --fieldName=price | richgauge"
