...
Start by creating a data ingestion stream. Something like:
   ...
   http://localhost:8080:> stream create --name springtweets --definition "twittersearch --consumerKey=<your_key> --consumerSecret=<your_secret> --query=spring | file --dir=/tweets/"

Next, create a tap on the _springtweets_ stream that sets a message counter named _tweetcount_

   http://localhost:8080:> tap create --name tweettap --definition "tap @ springtweets | counter --name=tweetcount"
...
To try this out, create a stream to ingest twitter feeds containing the word _spring_ and output to a file:
   ...
   http://localhost:8080:> stream create --name springtweets --definition "twittersearch --consumerKey=<your_key> --consumerSecret=<your_secret> --query=spring | file"

Now create a tap for a field value counter:

   http://localhost:8080:> tap create --name tweettap --definition "tap@springtweets | field-value-counter --fieldName=fromUser"
...
Creating an aggregate counter is very similar to a simple counter. For example, to obtain an aggregate count for our spring tweets stream:
    ...
    http://localhost:8080:> stream create --name springtweets --definition "twittersearch --query=spring | file"

you'd simply create a tap which pipes the input to `aggregatecounter`:

   http://localhost:8080:> tap create --name tweettap --definition "tap @ springtweets | aggregatecounter --name=tweetcount"
...
Create an ingest stream
    ...
    http://localhost:8080:> stream create --name test --definition "http --port=9090 | file"

Next create the tap:

    http://localhost:8080:> tap create --name simplegauge --definition "tap@test | gauge"

Now Post a message to the ingest stream:

    http://localhost:8080:> http post --target http://localhost:9090 --data "10"
...
Next create the tap:
      ...
      http://localhost:8080:> tap create --name testgauge --definition "tap@test | richgauge"

Now Post some messages to the ingest stream:

    http://localhost:8080:> http post --target http://localhost:9090 --data "10"
    http://localhost:8080:> http post --target http://localhost:9090 --data "13"
    http://localhost:8080:> http post --target http://localhost:9090 --data "16"
...
Create an ingest stream
     ...
     http://localhost:8080:> stream create --name stocks --definition "http --port=9090 | file"

Next create the tap, using the json-field-extractor to extract the stock price from the payload: 

     http://localhost:8080:> tap create --name stockprice --definition "tap@stocks | json-field-extractor --fieldName=price | richgauge"

Now Post some messages to the ingest stream:

    http://localhost:8080:> http post --target http://localhost:9000 --data "{\"symbol\":\"VMW\",\"price\":72.04}"
    http://localhost:8080:> http post --target http://localhost:9000 --data "{\"symbol\":\"VMW\",\"price\":72.06}"
    http://localhost:8080:> http post --target http://localhost:9000 --data "{\"symbol\":\"VMW\",\"price\":72.08}"
...
Next create the taps, using the json-field-extractor to extract the stock price from the payload: 
     ...
     http://localhost:8080:> stream create --name vmwprice --definition "tap@stocks |json-field-value-filter --fieldName=symbol --fieldValue=VMW| json-field-extractor --fieldName=price | richgauge"
     http://localhost:8080:> stream create --name emcprice --definition "tap@stocks |json-field-value-filter --fieldName=symbol --fieldValue=EMC| json-field-extractor --fieldName=price | richgauge"
...
Now Post some messages to the ingest stream:
    ...
    http://localhost:8080:> http post --target http://localhost:9000 --data "{\"symbol\":\"EMC\",\"price\":24.92}"
    http://localhost:8080:> http post --target http://localhost:9000 --data "{\"symbol\":\"EMC\",\"price\":24.90}"
    http://localhost:8080:> http post --target http://localhost:9000 --data "{\"symbol\":\"EMC\",\"price\":24.96}"
