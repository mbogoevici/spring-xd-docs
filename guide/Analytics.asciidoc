...
Next, create a tap on the _springtweets_ stream that sets a message counter named _tweetcount_
   ...
   http://localhost:8080:> stream create --name tweettap --definition "tap @ springtweets | counter --name=tweetcount"
...
Now create a tap for a field value counter:
   ...
   http://localhost:8080:> stream create --name tweettap --definition "tap@springtweets | field-value-counter --fieldName=fromUser"
...
Next create the tap:
    ...
    http://localhost:8080:> stream create --name simplegauge --definition "tap@test | gauge"
...
Next create the tap:
      ...
      http://localhost:8080:> stream create --name testgauge --definition "tap@test | richgauge"
...
Next create the tap, using the json-field-extractor to extract the stock price from the payload: 
     ...
     http://localhost:8080:> stream create --name stockprice --definition "tap@stocks | json-field-extractor --fieldName=price | richgauge"
