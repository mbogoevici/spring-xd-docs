...
To log the contents of a file create a stream defintion using the XD shell
...
The tail source has 4 optins

name:: the absolute path to the directory to monitor for files *(default: `/tmp/xd/input/<streamName>`)*
duplicates:: Default value is `true` to prevent the same file from being processed twice.  The list of processed files kept in memory
pattern:: A filter expression (Ant style) that accepts only file that match the pattern.
interval:: The fixed delay polling interval specified in seconds
