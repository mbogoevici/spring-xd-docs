...
readTimeout:: The timeout in ms for each read
connectTimeout: The timeout in ms for connecting to Twitter
language:: The language code (e.g., 'en') to include in the search
includeEntities:: Include entities (hashtag metadata, etc) - default is true
resultType:: popular, recent, mixed(default)
...
https://dev.twitter.com/docs/streaming-apis/parameters#language[language]:: comma delimited set of languages to include (defaults to `null`)
https://dev.twitter.com/docs/streaming-apis/parameters#follow[follow]:: comma delimited set of user ids whose tweets should be included in the stream (defaults to `null`)
