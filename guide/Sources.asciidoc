...
To create a stream definition in the server post using curl
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "http | file" http://localhost:8080/streams/httptest
...
Here is an example 
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "http --port=9020 | file" http://localhost:8080/streams/httptest9020
...
To create a stream definition post using curl
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tail | file" http://localhost:8080/streams/tailtest
...
Here is an example 
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tail --name=/tmp/foo --lines=5 | file --name=bar" http://localhost:8080/streams/tailtest
...
To create a stream definition in the server post using curl
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "twittersearch --consumerKey=<your_key> --consumerSecret=<your_secret> --query='#springone2gx' | file" http://localhost:8080/streams/springone2gx
...
TIP: For both `twittersearch` and `gardenhose` you can fill in in the `conf/twitter.properties` file instead of using the DSL parameters to supply keys and secrets.

[[gardenhose]]
==== Twitter "Gardenhose"

This source ingests data from Twitter's https://dev.twitter.com/docs/streaming-apis/streams/public[streaming] API. It uses the https://dev.twitter.com/docs/streaming-apis/streams/public[sample] stream rather than the full "firehose" which needs special access.
...
You need to supply all keys and secrets (both consumer and accessToken) to authenticate for this source, so it is easiest if you just add these to the `conf/twitter.properties` file. Stream creation is then straightforward:
     ...
     $ curl -d "gardenhose | file" http://localhost:8080/streams/hose
...
Here is an example. Create two streams: One to write http messages to a Gemfire region named _Stocks_, and another to execute the CQ.
      ...
      $ curl -X PUT -H 'Content-Type: text/plain' -d "http --port=9090 | gemfire-json-server --regionName=Stocks" --keyExpression=payload.getField('symbol')" http://localhost:8080/streams/stocks
      $ curl -X PUT -H 'Content-Type: text/plain' -d "gemfire-cq --query=Select * from /Stocks where symbol='VMW' | file" http://localhost:8080/streams/cqtest
...
To create a stream definition post using curl
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "syslog-udp --port=1514 | file" http://localhost:8080/streams/syslogtest

or

     $ curl -X PUT -H 'Content-Type: text/plain' -d "syslog-tcp --port=1514 | file" http://localhost:8080/streams/syslogtest
...
To create a stream definition in the server,  post using curl
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp | file" http://localhost:8080/streams/tcptest
...
.LF Decoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1235 --decoder=LF | file" http://localhost:8080/streams/tcptest2
...
.NULL Decoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1236 --decoder=NULL | file" http://localhost:8080/streams/tcptest3
...
.STXETX Decoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1237 --decoder=STXETX | file" http://localhost:8080/streams/tcptest4
...
.RAW Decoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1238 --decoder=RAW | file" http://localhost:8080/streams/tcptest5
...
.L1 Decoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1239 --decoder=L1 | file" http://localhost:8080/streams/tcptest6
...
.L2 Decoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1240 --decoder=L2 | file" http://localhost:8080/streams/tcptest7
...
.L4 Decoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1241 --decoder=L4 | file" http://localhost:8080/streams/tcptest8
...
==== Binary Data Example
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "tcp --port=1242 --decoder=L1 | file --binary=true " http://localhost:8080/streams/tcptest9
