...
Three syslog sources are provided: `syslog-udp`, `syslog-tcp` and `reactor-syslog`.  The reactor-syslog adapter uses tcp and builds upon the functionality available in https://github.com/reactor/reactor[Reactor] project and should provide improved throughput over the syslog-tcp adapter. They all support the following option:
...
reverse-lookup:: whether to attempt to resolve the host address *(default: `false`)*
