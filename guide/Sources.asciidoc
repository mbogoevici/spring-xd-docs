...
To create a stream definition (using shell command)
    ...
    xd:> stream create --name syslogtest --definition "syslog-udp --port=1514 | file" --deploy

or

    xd:> stream create --name syslogtest --definition "syslog-tcp --port=1514 | file" --deploy
...
[[reactor-tcp]]
=== Reactor TCP
The `reactor-tcp` source acts as a server and allows a remote party to connect to XD and submit data over a raw tcp socket.  The reactor-tcp source differs from the standard tcp source in that it is based on the https://github.com/reactor/reactor[Reactor Project] and can be configured to use an the http://martinfowler.com/articles/lmax.html[LMAX Disruptor RingBuffer] library allowing form extremebly high ingestion rates, e.g. ~ 1M/sec.

To create a stream definition use the following XD shell command

     xd:> stream create --name tcpReactor --definition "reactor-tcp | file" --deploy

This will create the default reactor TCP source and send data read from it to the file named tcpReactor.

The reactor-tcp source has the following options

framing:: `linefeed` or `length`.  How to frame the data to tell invidual messages apart. *(default: `linefeed`)*
...
lengthFieldLength:: length of the field to use when framing *(default: `4`)*
