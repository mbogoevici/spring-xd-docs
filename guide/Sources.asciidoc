...
[[gemfire]]
=== GemFire Cache Listener
This source configures a cache and replicated region in the XD container process along with a Spring Integration GemFire inbound channel adapter that outputs messages triggered by an external entry event on the region. By default the payload contains the updated entry value, but may be controlled by passing in a SpEL expression that uses the http://www.vmware.com/support/developer/vfabric-gemfire/700-api/com/gemstone/gemfire/cache/EntryEvent.html[EntryEvent] as the evaluation context.

==== Options

The qemfire source has the following options

region:: The name of the region for which events are to be monitored *(required, String)*
