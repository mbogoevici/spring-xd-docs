...
port:: the port on which the system will listen for syslog messages *(default: `1514`)*
...
[[reactor-tcp]]
=== Reactor TCP
The `reactor-tcp` source acts as a server and allows a remote party to connect to XD and submit data over a raw tcp socket.  The reactor-tcp source differs from the standard tcp source in that it is based on the https://github.com/reactor/reactor[Reactor Project] and can be configured to use an the http://martinfowler.com/articles/lmax.html[LMAX Disruptor RingBuffer] library allowing for an extremely high ingestion rates, e.g. ~ 1M/sec.
...
lengthFieldLength:: length of the field to use when framing. 2, 4 or 8.  *(default: `4`)*
