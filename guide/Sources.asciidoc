...
Post some data to the http server on the default port of 9000
     ...
     http://localhost:8080:>post httpsource --target http://localhost:9000 --data "hello world"
...
Post some data to the new port
    ...
    http://localhost:8080:>post httpsource --target http://localhost:9020 --data "hello world"
...
Now send some messages to the stocks stream.
     ...
     http://localhost:8080:>post httpsource --target http://localhost:9090 --data "{\"symbol\":\"VMW\", \"price\":78}"
     http://localhost:8080:>post httpsource --target http://localhost:9090 --data "{\"symbol\":\"VMW\", \"price\":80}"
