...
As an aid to avoid individual stream definitions getting too complex and hard to follow, XD allows stream composition
...
It can be broken into pieces using channel references:
...
Refering to the label `group1` then effectively refers to all the labeled modules. This is not
