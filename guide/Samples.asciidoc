...
Create the streams with syslog as source and HDFS as sink (Please refer to link:Sources#sources[source] and link:Sinks#sinks[sink])
  ...
  http://localhost:8080:> stream create --definition "syslog-udp --port=<udp-port> | hdfs" --name <stream-name>

  http://localhost:8080:> stream create --definition "syslog-tcp --port=<tcp-port> | hdfs" --name <stream-name>
