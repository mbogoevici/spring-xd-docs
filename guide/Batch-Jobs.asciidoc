...
When deploying batch job, like stream modules the deployment manifest can be provided.
It could be:
   * Count for the number of job modules
   * Criteria to deploy the job modules into matching containers
...
**Note:** When a batch job is launched/scheduled, the job execution is executed on only one of these modules. The job module that picks up the job launching request message executes the batch job. The **partitioning** support needs to be explicitly added to the step like https://github.com/spring-projects/spring-xd/wiki/Batch-Jobs#ftp-to-hdfs-export-ftphdfs[here]
...
Parameters for Job definition `host`, `port`, `username` and `password` can be used to control access to the FTP server.
