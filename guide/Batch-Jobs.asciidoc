...
----
xd:> stream create --name myTriggerStream --definition "trigger > :helloSpringXD"
----
In the logging output of the XDContainer you should see the following
----
...
----
 stream create --name jobStream --definition "http > :myHttpJob"
----
...
----
xd:> stream create --name cronStream --definition "cron-trigger --cron='0/5 * * * * *'  > :myCronJob" 

----
You can also pass in parameters to the batch job by using the --payload expression.
----
xd:> stream create --name cronStream --definition "cron-trigger --cron='0/5 * * * * *'  --payload='{"param1":"Kenny"}' > :myCronJob"  
...
----
xd:> stream create --name fdStream --definition "trigger --payload='{"param1":"fixedDelayKenny"}' --fixedDelay=10 > :myXDJob" 
