...
Performs the reverse of the previous module. The database configuration is the same as for `filejdbc` but without the initialization options. Since it's a assumed that you already have data to import to HDFS, when creating the job, you must either supply the select statement by setting the `sql` parameter, or you can supply both `tableName` and `columns` options (which will be used to build the SQL).
...
You can customize how the data is written to HDFS by supplying the options `basePath` (defaults to `/data/`), `filename` (defaults to job name), `rollover` (in bytes, default 1000000) and `suffix` (defaults to 'log').
