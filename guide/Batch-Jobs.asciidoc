...
A JobLauncher is responsible for starting a job and is often triggered via a scheduler.  Other options to launch a job are through Spring XD's RESTful administration API, the XD web application, or in response to an external event from and XD stream defintion, _e.g._ file polling using the file source.
...
To select specific listeners, specify comma seperated list in `--listeners` option. 
