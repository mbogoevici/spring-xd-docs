...
There are accessor methods that perform type conversion to the basic primitive types as well as BigDecimal and Date.  This avoid you having to cast the values to specific types and rely on the Tuple's type conversion infastructure to perform the conversion.   
...
There are several overloads for getters that let you provide default values for primitive types, Date formats, and Locale aware NumberFormat conversion.  A best effort has been made to preserve the functionality available in Spring Batch's FieldSet class that has been extensively used for parsing String based data in files.
...
There are a few other more generic methods available.  Their functionality shoudl be obvious from their names
