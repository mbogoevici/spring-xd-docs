...
*Modules* encapsulate a unit of work into a reusable component. Within the XD runtime environment Modules allow users to create definitions for link:Streams[Streams] as well as link:Batch-Jobs[Batch Jobs]. The *Modules* tab within the *Jobs* section allows users to create Batch Job definitions. In order to learn more about *Modules*, please see the chapter on link:Modules[Modules].
