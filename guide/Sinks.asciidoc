...
The full list of options available to the mail module is below:
  ...
  host:: The hostname of the sending server to use. *(default: look it up under the `mail.sink.host` key in the `mail.properties` file, with default `localhost`)*
  port:: The port of the sending server. *(default: look it up under the `mail.sink.port` key in the `mail.properties` file, with default `25`)*
  username:: The username to use for authentication against the sending server. *(default: look it up under the `mail.sink.username` key in the `mail.properties` file, with no default)*
  password:: The password to use for authentication against the sending server. *(default: look it up under the `mail.sink.password` key in the `mail.properties` file, with no default)*
...
xd:>stream create r --definition "http | router --expression=payload.contains('a')?':foo':':bar'"
