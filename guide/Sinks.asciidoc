...
To create a stream using a `jdbc` sink relying on all defaults you would use a command like
  ...
  xd:> stream create --name myjdbc --definition "time | jdbc --initializeDatabase=true"
