...
Probably the simplest option for a sink is just to log the data. The `log` sink uses the application logger to output the data for inspection. The log level is set to `WARN` and the logger name is created from the stream name. To create a stream using a `log` sink you would use a command like
  ...
  $ curl -X PUT -H 'Content-Type: text/plain' -d "http --port=8000 | log" http://localhost:8080/streams/mylogstream
...
The logger name is the sink name prefixed with the string "logger.". The sink name is the same as the stream name by default, but you can set it by passing the `--name` parameter 
  ...
  $ curl -X PUT -H 'Content-Type: text/plain' -d "http --port=8000 | log --name=mylogger" http://localhost:8080/streams/myotherlogstream
...
Another simple option is to stream data to a file on the host OS. This can be done using the `file` sink module to create a link:Streams#streams[stream].
  ...
  $ curl -X PUT -H 'Content-Type: text/plain' -d "http --port=8000 | file" http://localhost:8080/streams/myfilestream
...
You can cutomize the behavior and specify the `name` and `dir` properties of the output file. For example
  ...
  $ curl -X PUT -H 'Content-Type: text/plain' -d "http --port=8000 | file --name=myfile --dir=/some/custom/directory" http://localhost:8080/streams/otherfilestream
...
You should then be able to use the `hdfs` sink when creating a link:Streams#streams[stream]
  ...
  $ curl -X PUT -H 'Content-Type: text/plain' -d "http --port=8000 | hdfs --rollover=10" http://localhost:8080/streams/myhdfsstream
...
Now, configure a stream
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "time --interval=3 | tcp" http://localhost:8080/streams/tcptest
...
Create the stream, using the `L4` encoder
     ...
     $ curl -X PUT -H 'Content-Type: text/plain' -d "time --interval=3 | tcp --encoder=L4 --port=1235" http://localhost:8080/streams/tcptest
...
owner::
The owner of the tcp-port (default: admin)
tcp-port::
...
An example stream would be to take data from a twitter search and push it through to a splunk instance.
    ...
    $ curl -X PUT -H 'Content-Type: text/plain' -d "twittersearch --consumerKey= --consumerSecret= --query='#LOTR' | splunk" http://localhost:8080/streams/springone2gx
