...
The most simple deployment is to lauch an XD Container with an embedded XDAdmin from the <xd-install-dir>/xd/bin directory
...
You can start the XDContainer and XDAdmin servers individually
...
There is an additional configuration option to specify the location of the SpringXD install (Note, this will not be necessary when XD_HOME environment variable is supported)
