...
Download http://www.springsource.org/download/community?project=Spring%20XD[*spring-xd-1.0.0.M1-dist.zip*]
...
In a separate terminal, `cd` into the `shell` directory and start the XD shell, which you can use to issue commands. The shell is a more user-friendly front end to the REST API which Spring XD exposes to clients.

[source,sh]
----
shell/bin>$ ./spring-xd-shell
 _____                           __   _______
/  ___|          (-)             \ \ / /  _  \
...
Welcome to Spring XD Shell
http://localhost:8080:>
----

The shell prompt shows the URL of the currntly targeted Spring XD server. You should now be able to start using Spring XD.
