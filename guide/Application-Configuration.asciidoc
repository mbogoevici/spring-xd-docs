...
<3> When true, the bus will automatically declare dead letter queues and binding for each bus queue. The user is responsible for setting a policy on the broker to enable dead-lettering; see link:MessageBus#error-handling-message-delivery-failures[Message Bus Configuration] for more information.
...
<17> Determines which request headers will be transported
...
----
  messagebus:
    kafka:
      ...
      numOfKafkaPartitionsForCountEqualsZero:  10             # <3>
      default:
        batchingEnabled:           false      # <4>
        batchSize:                 200        # <5>
        batchTimeout:              5000       # <6>
        replicationFactor:         1          # <7>
        concurrency:               1          # <8>
        requiredAcks:              1          # <9>
...
<3> The number of Kafka partitions to use when module count can auto-grow. Should be bigger than the number of
containers that will ever exist.

<4> True to enable message batching by producers by default

<5> The number of messages in a batch (may be preempted by _batchTimeout_)

<6> The idle time to wait before sending a partial batch

<7> The replication factor of the topics created by the message bus. At least as many brokers must be in the cluster
when the topic is being created.

<8> The maximum number of consumer threads receiving messages for a module. The total number of threads actively
...
<9> The number of required acks when producing messages, i.e. how many brokers have committed data to the logs and
