...
The HSQLDB database is already configured as part of the default profile.  You can change the host, port and name used by uncommenting the following lines of configuration in `xd-config.yml` and edit the use the relevant values for your environment.
