...
For modules, each module has its own configuration file located in its own directory, for example `sink/http/http.properties`.  Shared configuration values for modules can be placed in a common `modules.yml` file.  
...
In this secton we will walk though how to configure servers and modules.
...
It is common to keep your server configuration seperate form the installation directory of XD itself.  To do this, here is an example environment variable setting 
...
NOTE: The `XD_MODULE_CONFIG_LOCATION` can refernce any any http://docs.spring.io/spring/docs/4.0.3.RELEASE/spring-framework-reference/htmlsingle/#resources[Spring Resource] implementation, most commonly denoted using the prefixes `classpath:`, `file:` and `http:`.

As an example, if you wanted to configure the twittersearch module, you would create a file 
----
XD_CONFIG_LOCATION\sink\twittersearch\twittersearch.properties
----

and the contents of that file would be property names such as `consumerKey` and `consumerSecret`.

NOTE: You *do not* need to prefix these property names with a `sink.twittersearch` prefix.
...
and `XD_CONFIG_LOCATION\sink\twitterstream\twitterstream.properties` contains
----
...
and `XD_CONFIG_LOCATION\sink\twittersearch\twittersearch.properties` contains
----
consumerKey=${sharedConsumerKey}
consumerSecret=${sharedConsumerSecret}
----
...
When resolving property file names, the server will look to load profile specific variants baed on the naming convention `<name>-{profile}.properties`.  For example, if given the OS environment variable `spring_profiles_active=default,qa` the following configuration file names for the twittersearch module would be searched in this order

. `XD_CONFIG_LOCATION\sink\twittersearch\twittersearch.properties`
. `XD_CONFIG_LOCATION\sink\twittersearch\twittersearch-default.properties`
. `XD_CONFIG_LOCATION\sink\twittersearch\twittersearch-qa.properties`
