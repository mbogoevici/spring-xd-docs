...
By default, the `xd-admin` server is unsecured and runs on an unencrypted HTTP connection. You can secure your administration REST endpoints, as well as the Admin UI by enabling HTTPS and requiring clients to authenticate.

===== Enabling HTTPS

By default, the administration, management, and health endpoints, as well as the Admin UI use HTTP as a transport. You can switch to HTTPS easily, by adding a certificate to your configuration.
...
will fail (so, for instance, make sure that you configure your Shell accordingly)
...
You can turn on authentication by adding the following to the configuration:
...
security:
  basic:
    enabled: true # false to disable security settings (default)      # <2>
    realm: SpringXD                                                   # <3>
...
<4> The username for authentication (must be used by REST clients and in the Admin UI)

<5> The password for authentication (must be used by REST clients and in the Admin UI)
