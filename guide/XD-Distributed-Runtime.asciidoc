...
First, use the +addresses+ property in +servers.yml+ to include the host/port for each server in the cluster. See link:Application-Configuration.html#rabbitConfig[Application Configuration].

By default, queues and exchanges declared by the bus are prefixed with +xdbus.+ (this prefix can be changed as described in link:Application-Configuration.html#rabbitBusProps[Application Configuration]).
