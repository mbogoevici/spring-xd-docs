...
First, use the +addresses+ property in +servers.yml+ to include the host/port for each server in the cluster. See <<Application-Configuration.asciidoc>>.

By default, queues and exchanges declared by the bus are prefixed with +xdbus.+ (this prefix can be changed as described in <<Application-Configuration.asciidoc>>).
