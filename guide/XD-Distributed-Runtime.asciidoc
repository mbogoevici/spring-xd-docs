...
The following properties are currently only implemented when using a _RabbitMessageBus_.
...
module.[modulename].consumer.backOffMaxInterval:: The maximum number of milliseconds to wait betwwen retries *(default 10000)*
...
module.[modulename].producer.partitionSelectorExpression:: A _SpEL_ expression, evaluated against the partition key, to determine the partition index to which the message will be routed. The final partition index will be the return value (an integer) modulo _producer.partitionCount_ If both the class and expression are null, the bus's default _PartitionSelectorStrategy_ will be applied to the key *(default null)* 
...
----
xd:>stream deploy --name partitioned --properties "module.jms.producer.partitionKeyExpression=payload.customerId, module.jms.producer.partitionSelectorExpression=hashCode(),module.transform.count=3"
...
----
xd:>stream deploy --name partitioned --properties "module.jms.producer.partitionKeyExtractionClass=foo.MyKeyExtractor, module.jms.producer.partitionSelectorClass=foo.MySelector,module.transform.count=3"

Deployed stream 'partitioned'
----

In this example three instances of the transformer will be created (with partition index of 0, 1, and 2). When the jms module sends a message it will pass the Message to an instance of _foo.MyKeyExtractor_ with the result being passed to an instance of _foo.MySelector_; the selector must return a value less than the _transform.count_ property so the message can be routed to one of the transformer instances.
