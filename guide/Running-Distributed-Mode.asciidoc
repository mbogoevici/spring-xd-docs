...
If you already have a running instance of *Redis* it can be used for Spring XD. By default Spring XD will try to use a _Redis_ instance running on *localhost* using *port 6379*.
...
Start the *RabbitMQ* broker by running the rabbit-server script:
[source,sh]
----
$ rabbitmq-server
----
...
Spring XD support the following Hadoop distributions:
