...
Rabbit MQ, Redis and Kafka for messaging during stream and job processing. A production installation must configure one
 of these transport options. At this time, Rabbit MQ is recommended for production scenarios, as Redis support is not
  intended to be used for production use, and Kafka support in XD is still under development and scheduled to be fully
  available by Spring XD 1.1.0.RELEASE. In either case, a separate server must be running to provide the messaging middleware.
...
* hadoop25 - Apache Hadoop 2.5.1 (default)
* phd21 - Pivotal HD 2.1 and 2.0
* cdh5 - Cloudera CDH 5.2.0 
